...
parser.add_argument("--debug_joycon", action="store_true", help="Print raw JoyCon axis/button values for remapping.")
...
def _show_camera_feed(env):
    """Display wrist and front camera feeds in separate OpenCV windows."""
    try:
        scene = env.unwrapped.scene
        frames = []
        for cam_key, win_name in [("wrist", "Wrist Camera"), ("front", "Front Camera")]:
            if cam_key in scene.sensors:
                rgb = scene.sensors[cam_key].data.output.get("rgb")
                if rgb is not None and rgb.shape[0] > 0:
                    img = rgb[0].cpu().numpy()  # (H, W, 4) or (H, W, 3)
                    img = img[:, :, :3]          # keep RGB only
                    img_bgr = img[:, :, ::-1].copy()
                    frames.append((win_name, img_bgr))
        if frames:
            combined = cv2.hconcat([f for _, f in frames]) if len(frames) > 1 else frames[0][1]
...
def main():  # noqa: C901
    ...
    elif args_cli.teleop_device == "joycon":
        from leisaac.devices import SO101JoyCon

        teleop_interface = SO101JoyCon(env, sensitivity=args_cli.sensitivity, debug=args_cli.debug_joycon)
    elif args_cli.teleop_device == "joycon-ee":
        from leisaac.devices import SO101JoyConEE

        teleop_interface = SO101JoyConEE(env, sensitivity=args_cli.sensitivity, debug=args_cli.debug_joycon)
