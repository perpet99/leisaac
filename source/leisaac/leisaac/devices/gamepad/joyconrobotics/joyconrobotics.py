...
class JoyconRobotics:
    def __init__(self, 
        ...
        # Allow standard serial, MAC address with colons, or 12-char hex string
        if device_serial != JOYCON_SERIAL_SUPPORT and self.joycon_id != None:
            serial_str = self.joycon_id[2]
            is_mac = len(serial_str) == 17 and serial_str.count(':') == 5
            is_hex = len(serial_str) == 12 and all(c in '0123456789abcdefABCDEF' for c in serial_str)
            if not (is_mac or is_hex):
